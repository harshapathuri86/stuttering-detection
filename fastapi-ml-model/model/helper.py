...
def run_model(model, audio):
    ...
    pks, vals, val_ind = theta_oscillator_main(
        audio, minfreq=50, maxfreq=7500, bands=10, Q_value=0.8, center_frequency=5, threshold=0.025, N_bands=30)
    ...
    yhat_splitted_list = np.array([np.array([np.count_nonzero(
        yhat_splitted[i]), len(yhat_splitted[i])]) for i in range(len(yhat_splitted))])

    yhat_splitted_list[:, 1] -= yhat_splitted_list[:, 0]

    # convert to list
    yhat_splitted_list = yhat_splitted_list.tolist()

    return yhat_splitted_list
